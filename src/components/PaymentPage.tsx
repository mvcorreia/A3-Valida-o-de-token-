...
const detectCardBrand = (number: string) => {
  ...
  if (/^(4011|4312|4389)/.test(cleaned)) {
    return "Elo";
  }
...
const PaymentPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (cardNumber.replace(/\D/g, "").length < 16) {
      toast.error("Digite um cartão com 16 números");

      return;
    }

    if (cardName.trim().length < 3) {
      toast.error("Digite o nome no cartão");
    ...
    if (cvv.length < 3) {
      toast.error("Digite um CVV válido");
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          setPage("token-validation");
        }, 700);
      } else {
        toast.error(data || "Erro ao processar pagamento");
      }
    } catch (error) {
